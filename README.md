...
- **Only hex color values are parsed.** Tokens whose value is `hsl()`, `oklch()`, raw channels, or an alias (`var(--other)`) are invisible to the *value* checks — which means a semantic token layer built on aliases, the very structure most worth adopting, is the structure this tool covers worst. Such a token declared in a dark block is reported as `dark-unparsed` rather than `missing-dark`: the override exists, and calling it "no dark entry" would be a wrong diagnosis rather than a cautious one.
- **`dark-handled-in-views` is decided by proximity, not by parsing.** A `dark:` variant within ~400 characters of the reference marks the token as handled. It cannot tell which element a declaration belongs to, so a token used in ten places is reclassified even if only one of them handles dark. The reclassification changes the finding's **kind only**; the severity stays `error`. Lowering it would let a CI gate pass a token that is genuinely unhandled in nine of its ten call sites — trading a false positive for a hidden defect, which is the worse of the two. Read it as "probably handled, confirm the consumers", not as a pass.
...
- **Tailwind v4 `@theme` is read as a light source, and semicolon-terminated at-rules are trimmed off selectors.** Both were needed because `@custom-variant dark (&:where(.dark, .dark *));` sits immediately before `@theme` in the standard v4 setup; absorbing that line as the next block's selector registered the entire theme as dark. Other preprocessor at-rules with unusual shapes may still confuse the selector split.
